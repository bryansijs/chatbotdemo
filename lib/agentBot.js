...
class Agent {
    ...
    start() {
        ...
        async.series([
            ...
            if (err) {
                ...
                console.log(chalk.blue('Agent checkForIncomingChats'));
    ...
    _loginAgent(callback) {
        ...
        request(options, (error, response, body) => {
            ...
            console.log(chalk.blue(`Agent login successfully. requestURL= ${this.requestURL}`));
            callback();
    ...
    _setAvailability(value, callback) {
        this.agentRequest(`PUT`, `availability`, { 'availability': { 'chat': value } }, err => {
            if (err) {
                ...
                console.log(chalk.blue(`Agent Availability has been set: ${value}`));
                callback();
            }
        });
    }
    ...
    checkForIncomingChats() {
        this.agentRequest('GET', 'incomingRequests', null, (err, body) => {
            if (err) {
                console.log(chalk.red(`Check for incomingRequests error: ${JSON.stringify(err)}`));
            }
            else {
                if (body && body.incomingRequests && (body.incomingRequests.ringingCount > 0)) {
                    console.log(chalk.blue('There is an incoming chat!'));
    ...
    startChat(chatURL) {
        ...
        chat.start((err) => {
            if (err) {
                ...
                console.log(chalk.blue(`Chat started successfully`));
            }
        });
    }
}
