...
class AgentChat {
    ...
    chatPolling(url) {
        ...
        ApiAiRequest.on('response', ((response) => {
            ...
            const newAmount = req.body.result.parameters.unit-currency[0].amount;
            apiAiResponseLine = apiAiResponseLine.replace( '[[newAmount]]', newAmount );
            apiAiResponseLine = apiAiResponseLine.replace( '[[amount]]', amount );

            //update amount
            if (response.result.action == "ChangeBBA.ChangeBBA-custom.ChangeBBA-Amount-custom"){
